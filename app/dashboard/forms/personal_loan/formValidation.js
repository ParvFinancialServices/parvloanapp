...
export const loanApplicationSchema = z
  ...
  .superRefine((data, ctx) => {
    ...
    if (
      data.have_offer_letter &&
      !(data.offer_letter instanceof File || typeof val == "string")
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
    ...
    if (
      data.has_bank_statement &&
      !(data.bank_statement instanceof File || typeof val == "string")
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
    ...
    if (data.has_current_loan) {
      if (
        !data.existing_loans ||
        isNaN(parseInt(data.existing_loans)) ||
        parseInt(data.existing_loans) < 0
      ) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message:
            "Number of existing loans is required and must be a non-negative number.",
          path: ["existing_loans"],
        });
      }
      ...
      if (
        !data.monthly_emi ||
        isNaN(parseFloat(data.monthly_emi)) ||
        parseFloat(data.monthly_emi) <= 0
      ) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
